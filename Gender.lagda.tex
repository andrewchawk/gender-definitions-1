...
For similar reasons, \AgdaRecord{Gender1} is all but incompatible with bigender \glspl{gEntity} and such; although \AgdaRecord{Gender1} \emph{does} \emph{technically} support the selection of multiple genders through comma-separated lists and such, the author finds that such second-class support is unfair and, therefore, unsuitable.  Also, like agender \glspl{gEntity}, an \(n\)-gender \gls{gEntity} \(e\), where \(n > 1\), may say that the gender identity of \(e\) is a combination of genders but \emph{is not actually a gender}; in this case, the simple \AgdaRecord{Gender1} approach is outright unsuitable.
