...
\begin{enumerate}
  \item \AgdaField{Gender.identificationExtent} \AgdaBound{g} \AgdaBound{s} is \AgdaInductiveConstructor{i0} if and only if for all \glspl{gEntity} \(x\), \AgdaBound{g} is the gender of \(x\) if and only if \(x\) does not at all identify with \AgdaBound{s}.
  \item \AgdaField{Gender.identificationExtent} \AgdaBound{g} \AgdaBound{s} is \AgdaInductiveConstructor{i1} if and only if for all \glspl{gEntity} \(x\), \AgdaBound{g} is the gender of \(x\) if and only if \(x\) completely identifies with \AgdaBound{s}.
...
The basic agender gender identity is simple in that this identity is materially equivalent to never identifying with any gender.  A bit more formally, one can say that for all \glspl{gEntity} \(x\), genders \(g\), and universe contexts \(U\), in \(U\), \(x\) does not at all identify with \(g\).  One who remembers that \AgdaInductiveConstructor{i0} is used to represent complete non-identification can translate that more formal definition like so:
