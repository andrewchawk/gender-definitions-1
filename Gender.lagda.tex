...
\setcounter{Sectionlevel}{2}
\newcounter{skipsubsubsection}
\setcounter{skipsubsubsection}{1}
...
\newenvironment{Section}[1]{
  ...
  \part{#1}
  \fi
  \ifnum\value{Sectionlevel}=2
  \chapter{#1}
  \fi
  \ifnum\value{Sectionlevel}=3
  \section{#1}
  \fi
  \ifnum\value{Sectionlevel}=4
  \subsection{#1}
  \fi
  %\ifnum\value{skipsubsubsection}=\value{1
  %\stepcounter{Sectionlevel}
  %\fi
  \ifnum\value{Sectionlevel}=5
  \subsubsection{#1}
  \fi
  \ifnum\value{Sectionlevel}=6
  \paragraph{#1}
  \fi
  \ifnum\value{Sectionlevel}=7
  \subparagraph{#1}
  \fi
}{
  \addtocounter{Sectionlevel}{-1}
...
\begin{Section}{On Not Defining ``Transgender''}\label{sec:nonGoalTransgender}

\begin{theorem}
  This paper does not formally define ``transgender''.
\end{theorem}
\begin{proof}
  This paper formally defines ``transgender'' only if this paper also defines a correspondence relation on the gender identities and the biological sexes; perhaps a touch informally, one can say that for all \glspl{gEntity} \(x\), \(x\) is transgender if and only if \(x\) has a body which has a biological sex which does \emph{not} correspond to the gender identity of \(x\).  Similarly, this paper defines such a correspondence relation only if this paper defines ``biological sex''.  However, biological sex is species-specific,\cite{mooreWhiteThroatedSparrow} so this paper defines ``biological sex'' only if this paper defines ``species''.  Also, the author thinks that many mainstream correspondences\footnote{Examples include XY complete gonadal dysgenesis, which is described in \textcite{wikipediaXyDysgenesis}.} between intersex genophenotypes and gender identities are arbitrary, so this paper does not define said correspondence at all.  Therefore, this paper does not formally define ``transgender''.
\end{proof}
\end{Section}
\end{Section}

\begin{Section}{Miscellaneous Stuff}

\begin{Section}{Stuff for Agda}

\begin{Section}{Options}
The following Agda blip indicates that this whole document is safe, i.e., does not use \AgdaKeyword{postulate}s or anything of the sort.
...
\begin{Section}{The Module Header}
...
\begin{Section}{Recommended Reading}

\begin{Section}{Agda Primitives}
As one may have expected, this document uses some Agda primitives.  Although using such primitives without explicit ``\AgdaKeyword{import}'' statements is possible, such usage demands the use of fully-qualified identifiers, and the resulting verbosity irritates the author, who, accordingly, has decided to use some ``\AgdaKeyword{open} \AgdaKeyword{import}'' statements to decrease such verbosity.  Anyway, a brief listing or such of the primitives which are used is as follows:
...
\begin{Section}{The Agda Standard Library}
...
\begin{Section}{Supporting Types}

\begin{Section}{The Unit Interval Type}
Basically, \AgdaDatatype{I} is the real-valued unit interval type.  More formally, one can say that for all values \AgdaBound{x} of type \AgdaDatatype{I}, \AgdaBound{x} is --- or represents, according to \emph{some people} --- some real number which is less than or equal to one and is greater than or equal to zero.

\AgdaInductiveConstructor{from-rational} is good for constructing simple \AgdaDatatype{I} terms; \AgdaInductiveConstructor{from-rational} \AgdaBound{q} \AgdaBound{lt1} \AgdaBound{lt2} is numerically equal to \AgdaBound{q}.
...
\begin{Section}{Extremes}
\AgdaFunction{i0} and \AgdaFunction{i1}, which are numerically equal to zero and one, respectively, are defined as follows:
...
\begin{Section}{The Equality Relation}
Basically, \AgdaFunction{IEq} is the \AgdaDatatype{I}-specific equality-type, i.e., \AgdaBound{x} \AgdaBound{y} is populated if and only if \AgdaBound{x} is numerically equal to \AgdaBound{y}.  \AgdaFunction{IEq} is defined as follows:

\begin{code}
module IEq where
  data IEq : I → I → Set where
    Structural : {x y : I} → x ≡ y → IEq x y
...
\begin{Section}{A Particularly Simple Definition}

\begin{code}
module Attempt1 where
\end{code}

\begin{Section}{Summary, what with the Spoilers and Such}
...
\begin{Section}{Definition}\label{sec:gender1definition}
...
\begin{example}\label{example:gender1exampleFemale}
  ...
  One issue which is immediately obvious to the author is that \hyperref[enum:gender1stringComparison]{\AgdaPostulate{String} might be a pain for determining equivalences between genders}.
\end{example}
...
\begin{Section}{Flaws}\label{sec:gender1flaws}
...
\begin{Section}{\ecapitalisewords{\glspl{gEntity}} whose Gender Count Is Not One}\label{sec:gender1non1gender}
...
\end{Section}
\begin{Section}{\(n\)-Gender Support}
...
\begin{Section}{Vagueness}\label{sec:gender1vagueness}
...
\begin{Section}{A More Agender-Friendly Type}

\begin{code}
module Attempt2 where
\end{code}

\begin{Section}{Summary, what with the Spoilers and All}
...
\begin{Section}{Definition}\label{sec:gender2definition}
...
\begin{Section}{Flaws}\label{sec:gender2flaws}
...
\begin{Section}{A More \(n\)-Gender-Friendly Approach}
...
\begin{Section}{Definition}\label{sec:gender3definition}
...
\begin{Section}{Flaws}\label{sec:gender3flaws}
...
\begin{Section}{Genderfluid-Polygender Distinction Failure}\label{sec:gender3DistinctionFailure}
...
\begin{Section}{A More Genderfluid-Friendly Definition}
...
\begin{Section}{Definition}\label{sec:gender4definition}
...
Much like the simpler \(P\), \AgdaFunction{AbstractGender} can be semantically defined as follows: For all universe levels \AgdaBound{u}, \AgdaBound{u}-universe-context types \AgdaBound{UC}, \AgdaFunction{AbstractGender} \AgdaBound{UC} terms \AgdaBound{G}, and \glspl{gEntity} \(x\), \AgdaBound{G} is the gender identity of \(x\) if and only if for all universe contexts \AgdaBound{U} and genders \AgdaBound{g}, ``\AgdaBound{g} is a gender of \(x\) in context \AgdaBound{U}'' holds if and only if \AgdaBound{G} \AgdaBound{U} \AgdaBound{g} is inhabited.
\end{Section}

\begin{Section}{Flaws}\label{sec:gender4flaws}
...
\begin{Section}{Extent}\label{sec:gender4Extent}
...
\begin{Section}{A Similar --- but Interval-Based --- Approach}\label{sec:gender5}

\begin{code}
module Attempt5 where
\end{code}

\begin{Section}{A Crude --- and Spoiler-Inclusive --- Summary}
...
\begin{Section}{Definition}\label{sec:gender5definition}
...
For \AgdaFunction{AbstractGender}, one can \emph{so informally} say that for all levels \AgdaBound{u}, \glspl{gEntity} \(x\), \AgdaBound{u}-universe-context types \AgdaBound{UC}, and \AgdaFunction{AbstractGender} \AgdaBound{UC} terms \AgdaBound{G}, \AgdaBound{G} is the gender identity of \(x\) if and only if for all relevant universe-contexts \AgdaBound{U} and genders \AgdaBound{g}, the extent to which \(x\), in \AgdaBound{U}, identifies with \AgdaBound{g} is \AgdaBound{G} \AgdaBound{U} \AgdaBound{g}, with \AgdaFunction{0ℚ} representing a complete lack of identification with a given gender and \AgdaFunction{1ℚ} representing total identification with a given gender.
\end{Section}

\begin{Section}{Flaws}\label{sec:gender5flaws}
...
\begin{Section}{Definition}\label{sec:gender6definition}
...
Once again, the Agda version is essentially just a universe-polymorphic equivalent of the non-Agda function.  More formally, one, e.g., the author, who is sick of writing this boilerplate-like stuff, can say that for all \glspl{gEntity} \(x\), levels \AgdaBound{u}, \AgdaBound{u}-universe-context types \AgdaBound{UC}, and \AgdaFunction{AbstractGender} \AgdaBound{UC} terms \AgdaBound{G}, \AgdaBound{G} is the gender identity of \(x\) if and only if for all appropriate \AgdaBound{U} and \AgdaBound{g}, the extent to which \(x\) identifies with \AgdaBound{g} in \AgdaBound{U} is \AgdaBound{G} \AgdaBound{U} \AgdaBound{g}, where \AgdaFunction{i0} and \AgdaFunction{Agdai1} indicate complete non-identification and total identification with \AgdaBound{g}, respectively, and other \AgdaDatatype{I} values are assigned linearly.
\end{Section}

\begin{Section}{Flaws}\label{sec:gender6flaws}
Some flaws of this approach are as follows:
...
\begin{Section}{An Alternative Type for Gender}\label{sec:gender7}
...
\begin{Section}{A New Type for Gender}
...
\begin{Section}{The Gender Identity Type}
...
\begin{Section}{Advantages}\label{sec:gender7advantages}
...
\begin{Section}{Flaws}\label{sec:gender7flaws}
Some flaws of this approach are as follows:
...
\begin{Section}{Extent}\label{sec:gender7flawExtent}
\begin{theorem}
...
\begin{Section}{The Interval Type for Gender}

\begin{code}
module Attempt8 where
\end{code}

\begin{Section}{Summary, Spoilers, and Such}
This \AgdaFunction{AbstractGender} is \emph{very} similar to \hyperref[sec:gender7]{\AgdaFunction{Attempt7.AbstractGender}}, but \AgdaRecord{Gender.selfDescription} is a function from the gender self-descriptions to the unit interval, with the output indicating the extent of identification with the input gender self-description.  This approach \summaryLink{enum:gender8advantageExtent}{solves the extent problem} but \summaryLink{enum:gender8flawDysphoria}{fails to distinguish between dysphoria-inducing non-identification and non-dysphoria-inducing non-identification}.
\end{Section}

\begin{Section}{Definitions}

\begin{Section}{Yet Another Type for Gender}
...
\begin{Section}{A More Formal Semantic Definition of \AgdaRecord{Gender}}
...
\begin{enumerate}
  \item \AgdaField{Gender.identificationExtent} \AgdaBound{g} \AgdaBound{s} is \AgdaFunction{i0} if and only if for all \glspl{gEntity} \(x\), \AgdaBound{g} is the gender of \(x\) if and only if \(x\) does not at all identify with \AgdaBound{s}.
  \item \AgdaField{Gender.identificationExtent} \AgdaBound{g} \AgdaBound{s} is \AgdaFunction{i1} if and only if for all \glspl{gEntity} \(x\), \AgdaBound{g} is the gender of \(x\) if and only if \(x\) completely identifies with \AgdaBound{s}.
...
\begin{Section}{The Basic Pangender-Identity}\label{sec:gender8exampleIdentityPangenderBasic}
Where \(P\) denotes the pangender gender-identity which is particularly basic, \(P\) is such that for all \glspl{gEntity} \(x\), \(P\) is the gender identity of \(x\) if and only if in every appropriate universe-context, \(P\) completely identifies with every gender.  Much like \AgdaFunction{agender} of \fref{sec:gender8exampleIdentityAgender}, translating this representation is pretty easy.  Accordingly, the author has structurally defined \AgdaFunction{basicPangender}, which corresponds to \(P\), as follows:
...
\begin{Section}{The Male Identity}\label{sec:gender8exampleIdentityMale}
Formally, one can say that the male gender-identity, i.e., ``abstract gender'', can be represented as a function which, for all relevant universe contexts, returns one or zero if the input gender is the male gender or is not the male gender, respectively.  Listing all self-descriptions of the male gender is beyond the scope of this paper,\footnote{Such listing is beyond the scope \emph{for now}.} so the author has chosen to use a \AgdaKeyword{module}-based approach.  Basically, there exists a type function \AgdaRecord{MaleIdentity.Correct} such that for any given gender identity \AgdaBound{G}, universe level \AgdaBound{u}, and \AgdaBound{u}-level-universe-context type \AgdaBound{UC}, \AgdaRecord{MaleIdentity.Correct} \AgdaBound{UC} \AgdaBound{male} \AgdaBound{G} is populated if and only if \AgdaBound{G} is the male gender-identity, where \AgdaBound{male} is the male gender.  \AgdaRecord{MaleIdentity.Correct} is defined as follows:
...
\begin{code}
    module MaleIdentity
      ...
      Correct : AbstractGender UC → Set u
      Correct maleIdentity = ((uc : UC) → IEq i1 (maleIdentity uc male))
                           × (∀ g uc → g ≢ male → IEq i0 (maleIdentity uc g))
\end{code}
\end{Section}

\begin{Section}{The Female Identity}\label{sec:gender8exampleIdentityFemale}
\AgdaModule{FemaleIdentity} is pretty much just \AgdaModule{MaleIdentity}, but ``female'' has replaced ``male''.  Once again, this module is so similar that the author recommends just reading the stuff in \fref{sec:gender8exampleIdentityMale}.
...
\begin{code}
    module FemaleIdentity
      ...
      Correct : AbstractGender UC → Set u
      Correct femaleIdentity = ((uc : UC) → IEq i1 (femaleIdentity uc female))
                             × (∀ g uc → g ≢ female → IEq i0 (femaleIdentity uc g))
\end{code}
\end{Section}

\begin{Section}{The Generic One-Gender Identity}
As indicated by the sarcastic not-a-sematic-definition in \fref{sec:gender8exampleIdentityFemale}, \AgdaFunction{AbstractGender}'s representations of the female gender-identity and the male gender-identity are very similar --- in fact, only the names of the variables differ.  Accordingly, one, e.g., the author, can abstract the variable names to indicate that \AgdaFunction{Correct} can hold proofs regarding \emph{any} appropriate gender-identity's being a one-gender gender-identity.  Accordingly still, the author has defined \AgdaFunction{OneGenderIdentity} as follows:

\begin{code}
    module _ {u : Level} {UC : Set u} where
      OneGenderIdentity : Gender → AbstractGender UC → Set u
      OneGenderIdentity g G =
        ∀ uc → (IEq i1 (G uc g)) × (∀ h → h ≢ g → IEq i0 (G uc h))
\end{code}
\end{Section}

\begin{Section}{Genderfluid Verification}
If\footnote{This ``if'' structuring replaced an ``assuming that'' structuring because the author thought that the ``if'' sentence would be more easily parsed.  The ``if'' still implies assumption.} \(G\) is a gender identity such that for all \glspl{gEntity} \(x\), if \(G\) is the gender identity of \(x\), then \(x\) totally identifies with gender \(g_1\), \(g_2\), and \(g_3\) at contexts \(C_1\), \(C_2\), and \(C_3\), respectively, but does not identify with any gender in any context \(c\) for \(C\ c\), then one can write a set of constraints which describe \(G\) like so:
...
\begin{code}
    module GenderFluidRestriction1 where
      module Constraints
        ...
        Correct G = IEq i1 (G c1 g1)
                  × IEq i1 (G c2 g2)
                  × IEq i1 (G c3 g3)
                  × ((c : UC) → C c → (g : Gender) → IEq i0 (G c g))
\end{code}
\end{Section}

\begin{Section}{Genderfluid Verification 2}
...
\begin{code}
    module GenderFluidRestriction2 where
      module Constraints
        ...
        Correct : AbstractGender UC → Set (u ⊔ v)
        Correct G = All (λ (c , g) → IEq i1 (G c g)) l
                  × (∀ g c → C c → IEq i0 (G c g))
\end{code}
\end{Section}

\begin{Section}{A More Generic Pangender Type}
In contrast with the pangender term of \fref{sec:gender8exampleIdentityPangenderBasic}, one can say that for any given \glspl{gEntity} \(x\), the gender identity of \(x\) is a pangender identity if and only if for any universe context \(u\) and gender \(g\), in \(u\), the extent to which \(x\) identifies with \(g\) is nonzero.  Using Agda, one can write a more succinct definition like so:

\begin{code}
    Pangender : {u : Level} → {UC : Set u} → AbstractGender UC → Set u
    Pangender G = ∀ g c → ¬ IEq i0 (G g c)
\end{code}

\begin{Section}{Proof of \AgdaFunction{basicPangender}'s Pangender Nature}
Also, using \AgdaFunction{Pangender}, one can prove that \AgdaFunction{basicPangender} terms \emph{do} actually represent pangender identities.  One such proof is as follows:

\begin{code}
    basicPangender-is-pangender-identity :
      {u : Level} → {UC : Set u} → Pangender (basicPangender UC)
    basicPangender-is-pangender-identity g c (IEq.Structural ())
\end{code}
\end{Section}
\end{Section}
\end{Section}
...
\begin{itemize}
  \item This approach does not explicitly distinguish between complete non-identification and dysphoria-inducing non-identification.\label{enum:gender8flawDysphoria}
\end{itemize}
\end{Section}
\end{Section}
