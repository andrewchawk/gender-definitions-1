...
\begin{enumerate}
  \item \AgdaField{Gender.selfDescriptions} \AgdaBound{g} \AgdaBound{s} is \AgdaInductiveConstructor{Agda.Primitive.Cubical.i0} if and only if for all \glspl{gEntity} \(x\), \AgdaBound{g} is the gender of \(x\) if and only if \(x\) does not at all identify with \AgdaBound{s}.
  \item \AgdaField{Gender.selfDescriptions} \AgdaBound{g} \AgdaBound{s} is \AgdaInductiveConstructor{Agda.Primitive.Cubical.i1} if and only if for all \glspl{gEntity} \(x\), \AgdaBound{g} is the gender of \(x\) if and only if \(x\) completely identifies with \AgdaBound{s}.
