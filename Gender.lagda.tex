...
Alternatively, gender identity can be represented as a binary predicate \(P\) on the universe contexts and the genders such that for all such gender identities \(P\), universe contexts \(U\), and genders \(g\), there exists a term of type \(P\ U\ g\) if and only if for all given \gls{gEntity} \(x\) of gender identity \(P\), \(g\) is a gender of \(x\) when \(x\)'s universe context is \(U\).  \AgdaFunction{AbstractGender}, which is a universe-level-polymorphic version of \(P\), is defined as follows:
